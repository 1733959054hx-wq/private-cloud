...
public class GlmService {
    ...
    public String summarizeDocument(String documentContent) throws IOException {
        ...
        requestBody.put("model", glmConfig.getFreeModel());
    ...
    private String callApi(JSONObject requestBody) throws IOException {
        Request request = new Request.Builder()
                .url(glmConfig.getBaseUrl() + "/chat/completions")
                .addHeader("Authorization", "Bearer " + glmConfig.getApiKey())
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new IOException("GLM API调用失败: " + response.code() + " - " + errorBody);
            }
    ...
    public SseEmitter chatStream(List<Map<String, String>> historyMessages) {
                ...
                try (Response response = client.newCall(request).execute()) {
                    if (!response.isSuccessful()) {
                        emitter.send(SseEmitter.event().name("error")
                                .data("GLM API调用失败: " + response.code()));
                        emitter.complete();
                        return;
                    }
