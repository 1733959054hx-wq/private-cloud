...
public class MimoService {
    ...
    public String summarizeDocument(String documentContent, String modelType) throws IOException {
        String model = "pro".equalsIgnoreCase(modelType)
                ? mimoConfig.getProModel()
                : mimoConfig.getFlashModel();
    ...
    private String callApi(JSONObject requestBody) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                ...
                throw new IOException("MiMo API调用失败: " + response.code() + " - " + errorBody);
            }
    ...
    public SseEmitter chatStream(List<Map<String, String>> historyMessages, String modelType) {
        ...
        new Thread(() -> {
            try {
                ...
                try (Response response = client.newCall(request).execute()) {
                    if (!response.isSuccessful()) {
                        emitter.send(SseEmitter.event().name("error")
                                .data("MiMo API调用失败: " + response.code()));
                        emitter.complete();
                        return;
                    }
                ...
                try {
                    emitter.send(SseEmitter.event().name("error")
                            .data("MiMo API调用失败: " + e.getMessage()));
